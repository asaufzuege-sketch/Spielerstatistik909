// app.js
// Vollständige Anwendung: Header entfernt, Top-Bar wird pro Seite zentriert eingesetzt.
// Alle Buttons behalten ihre IDs und Funktionen; Marker, Time-Tracking, GoalValue, Season,
// CSV Import/Export, Timer, Reset sind vollständig implementiert.

document.addEventListener("DOMContentLoaded", () => {
  // --- Seiten / Page-Container ---
  const pages = {
  ...
  // --- Top-Bar (Buttons) erstellen und wiederverwenden ---
  let topBarEl = null;
  function createTopBarOnce() {
    if (topBarEl) return;
  ...
  function placeTopBarForPage(pageKey) {
    createTopBarOnce();
    if (!topBarEl) return;
    if (topBarEl.parentElement) topBarEl.parentElement.removeChild(topBarEl);
    if (pageKey === "selection") return; // no buttons on selection page
    const pageEl = pages[pageKey];
    if (!pageEl) {
      document.body.insertBefore(topBarEl, document.body.firstChild);
      return;
    }
    pageEl.insertBefore(topBarEl, pageEl.firstChild);
  }

  // --- Erstelle TopBar bevor Elemente abgefragt werden ---
  createTopBarOnce();

  // --- Query Elemente (Buttons existieren weil TopBar erstellt wurde) ---
  const playerListContainer = document.getElementById("playerList");
  const confirmSelectionBtn = document.getElementById("confirmSelection");
  const statsContainer = document.getElementById("statsContainer");
  // Buttons (may be relocated in DOM, but still queryable by ID)
  ...
  let seasonData = JSON.parse(localStorage.getItem("seasonData")) || {};
      ...
      .forEach((p, idx) => {
        ...
        li.innerHTML = `
          <label class="player-line" style="display:flex;align-items:center;gap:8px;width:100%;" for="${checkboxId}">
            <input id="${checkboxId}" name="${checkboxId}" type="checkbox" value="${p.name}" ${checked} style="flex:0 0 auto">
            ${numAreaHtml}
    ...
    for (let i = 0; i < 5; i++) {
      ...
      li.innerHTML = `
        <label class="custom-line" style="display:flex;align-items:center;gap:8px;width:100%;" for="${chkId}">
          ...
          <input id="${numId}" name="${numId}" type="text" class="custom-num" inputmode="numeric" maxlength="3" placeholder="Nr." value="${pre?.num || ""}" style="width:56px;flex:0 0 auto;">
  ...
  // --- Buttons & import setup (keeps previous behavior) ---
  (function setupButtonsAndImports() {
    ...
    // Apply colors if elements exist (they will exist after topBar inserted)
    const tryApplyColors = () => {
      if (exportSeasonFromStatsBtn()) { exportSeasonFromStatsBtn().style.backgroundColor = colorExportSeason; exportSeasonFromStatsBtn().style.color = "#000"; }
      if (exportSeasonMapBtn()) { exportSeasonMapBtn().style.backgroundColor = colorExportSeason; exportSeasonMapBtn().style.color = "#000"; }
      if (seasonBtn()) { seasonBtn().style.backgroundColor = colorSeason; seasonBtn().style.color = "#fff"; }
      if (seasonMapBtn()) { seasonMapBtn().style.backgroundColor = colorSeasonMap; seasonMapBtn().style.color = "#fff"; }
      if (exportBtn()) { exportBtn().style.backgroundColor = colorExportCSV; exportBtn().style.color = "#fff"; }
    };

    // Try once now (IDs present), and again after slight delay (in case topBar is re-inserted)
    tryApplyColors();
    setTimeout(tryApplyColors, 60);
    ...
    // Insert an Import CSV button next to export button if present
    const intervalTry = setInterval(() => {
      const ref = exportBtn();
      const resetRef = resetBtn();
      if (ref && resetRef) {
        if (!document.getElementById("importCsvStatsBtn")) {
          const importStatsBtn = createImportButton("importCsvStatsBtn", "Import CSV", ref, resetRef, null);
  ...
  function createMarkerBasedOn(pos, boxEl, longPress, forceGrey=false) {
    ...
    if (boxEl.classList.contains("field-box")) {
      ...
      if (img) {
        ...
        if (longPress || forceGrey) { createMarkerPercent(pos.xPctContainer, pos.yPctContainer, "#444", boxEl, true); return; }
        if (sampler && sampler.valid) {
          const ix = pos.xPctImage;
          const iy = pos.yPctImage;
          if (sampler.isGreenAt(ix, iy, 110, 30)) { createMarkerPercent(pos.xPctContainer, pos.yPctContainer, "#00ff66", boxEl, true); return; }
          if (sampler.isRedAt(ix, iy, 95, 22)) { createMarkerPercent(pos.xPctContainer, pos.yPctContainer, "#ff0000", boxEl, true); return; }
          return;
        } else {
    ...
    if (boxEl.classList.contains("goal-img-box") || boxEl.id === "goalGreenBox" || boxEl.id === "goalRedBox") {
      ...
      if (boxEl.id === "goalGreenBox") {
        ...
        createMarkerPercent(pos.xPctContainer, pos.yPctContainer, "#444", boxEl, true); return;
      }
  ...
  attachMarkerHandlersToBoxes("#torbildPage .field-box, #torbildPage .goal-img-box");
  attachMarkerHandlersToBoxes("#seasonMapPage .field-box, #seasonMapPage .goal-img-box");
  ...
  function exportSeasonMapFromTorbild() {
    const boxes = Array.from(document.querySelectorAll("#torbildPage .field-box, #torbildPage .goal-img-box"));
  ...
  function renderSeasonMapPage() {
    const boxes = Array.from(document.querySelectorAll("#seasonMapPage .field-box, #seasonMapPage .goal-img-box"));
  ...
  function startTimer(){
    if (!timerInterval) {
      ...
      timerRunning = true;
      const tb = timerBtn();
      if (tb) { tb.classList.remove("stopped","reset"); tb.classList.add("running"); }
    }
  }
  function stopTimer(){
    ...
    timerRunning = false;
    const tb = timerBtn();
    if (tb) { tb.classList.remove("running","reset"); tb.classList.add("stopped"); }
  }
  function resetTimerOnlyClock(){
    ...
    updateTimerDisplay();
    const tb = timerBtn();
    if (tb) { tb.classList.remove("running","stopped"); tb.classList.add("reset"); }
  }

  let holdTimer = null, longPress = false;
  const LONG_MS = 800;
  // Bind timer button when it exists (topBar can move, so bind via event delegation: rebind on insertion)
  function bindTimerButton() {
    const tb = timerBtn();
    if (!tb) return;
    // remove previous listeners by cloning
    const n = tb.cloneNode(true);
    tb.parentNode.replaceChild(n, tb);

    n.addEventListener("mousedown", () => { longPress=false; holdTimer = setTimeout(()=>{ resetTimerOnlyClock(); longPress=true; }, LONG_MS); });
    n.addEventListener("mouseup", () => { if (holdTimer) clearTimeout(holdTimer); });
    n.addEventListener("mouseleave", () => { if (holdTimer) clearTimeout(holdTimer); });
    n.addEventListener("touchstart", () => { longPress=false; holdTimer = setTimeout(()=>{ resetTimerOnlyClock(); longPress=true; }, LONG_MS); }, {passive:true});
    n.addEventListener("touchend", () => { if (holdTimer) clearTimeout(holdTimer); });
    n.addEventListener("touchcancel", () => { if (holdTimer) clearTimeout(holdTimer); }, {passive:true});
    n.addEventListener("click", () => { if (longPress) { longPress=false; return; } if (timerInterval) stopTimer(); else startTimer(); });
  }

  // Rebind timer button whenever topBar is placed
  const topBarObserver = new MutationObserver(() => {
    bindTimerButton();
    // also re-apply other colors or dynamic import buttons if necessary
  });
  ...
  // NOTE: resetTorbildBtn, resetSeasonBtn, resetSeasonMapBtn are not top-bar buttons in this layout;
  // if present in page markup they should have IDs and handlers; we rely on existing markup if used.
  ...
  function showPage(pageKey) {
    ...
    placeTopBarForPage(pageKey);

    if (pageKey === "stats") {
      renderStatsTable();
      updateIceTimeColors();
    } else if (pageKey === "season") {
      renderSeasonTable();
    } else if (pageKey === "seasonMap") {
      renderSeasonMapPage();
    } else if (pageKey === "goalValue") {
      renderGoalValuePage();
    }
  }
  window.showPage = showPage;

  // --- Wire top-level navigation (IDs exist on topBar) ---
  function wireTopBarNav() {
    const selBtn = selectPlayersBtn();
    if (selBtn) selBtn.addEventListener("click", () => showPage("selection"));

    const torBtn = torbildBtn();
    if (torBtn) torBtn.addEventListener("click", () => showPage("torbild"));

    const gvBtn = goalValueBtn();
    if (gvBtn) gvBtn.addEventListener("click", () => showPage("goalValue"));

    const sBtn = seasonBtn();
    if (sBtn) sBtn.addEventListener("click", () => { showPage("season"); });

    const smBtn = seasonMapBtn();
    if (smBtn) smBtn.addEventListener("click", () => { showPage("seasonMap"); });

    // export buttons are wired where implemented (exportSeasonHandler etc.)
  }

  // Re-wire topBar nav whenever topBar inserted
  const placeObserver = new MutationObserver(() => {
    wireTopBarNav();
    bindTimerButton();
  });
